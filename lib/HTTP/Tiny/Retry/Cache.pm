...
sub request {
  ...
  $self->{cache_dir}     //= $ENV{HTTP_TINY_CACHE_DIR}
    // path( File::Spec->tmpdir, "http-tiny-cache-" . path($0)->basename )->realpath;
...
sub clean_cache {
  ...
  if ($clean_all) {
    ...
    for my $cachefile ( path( $self->{cache_dir} )->children(qr{\.json$}x) ) {
      try {
  ...
  return scalar( grep { -d $_ } path( $self->{cache_dir} )->children );
}
